 ...
 *
 * SETUP — Add these in Cloudflare Pages → Settings → Environment Variables:
 *
 ...
 *
 * Cloudflare Pages Functions automatically route requests to
 * /api/verify-license → this file. No extra config needed.
 *
 * DEVICE LIMIT STRATEGY (no database needed):
 * - Peek at Gumroad's uses counter without incrementing
 * - If uses < 3, increment to consume one activation slot
 * - If uses >= 3, return limitReached so the client falls back
 *   to its localStorage cache (already-verified devices keep working)
 */
...
// ── Main export — Cloudflare Workers / Pages Functions format ──────────────
export default {
  async fetch(request, env) {
    ...
    // Resolve product ID from env — keeps real IDs out of source code
    const PRODUCT_IDS = {
    ...
    try {
      ...
      if (currentUses >= DEVICE_LIMIT) {
        return json({
          ...
          limitReached: true,
          uses:         currentUses,
          error:        `This license key has been activated on ${DEVICE_LIMIT} devices already — the maximum allowed. Email hello@kreainvoice.com if you need help.`,
        });
      }
